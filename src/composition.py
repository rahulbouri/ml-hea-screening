...
def _assign_features(matrices, elem_info, formulae, sum_feat=False):
    ...
    if sum_feat:
        ...
        feats = np.concatenate([avg_feats, range_feats, var_feats], axis=1) ##### returning averge, range and variance 
...
def generate_features(df, elem_prop='oliynyk',
    ...
    #                          + elem_prop
    #                          + '.csv')
    elem_props = pd.read_csv("/Users/rahulbouri/Desktop/ml_hea/Nballoy_BO/cbfv/element_properties/f3_revised.csv")
    ...
    column_names = np.concatenate(['avg_' + elem_props_columns,
                                   'range_' + elem_props_columns,  ############## Swapped range and variance 
